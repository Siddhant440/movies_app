...
AI was used mainly to help debug the flow of state management, keep the architecture understandable, and reduce friction while implementing local-first behavior. The assistance was especially useful for tracing how Riverpod providers, screen widgets, and the SQLite layer interact when a user saves a movie, creates a profile, or triggers sync.

The main focus was on:

- debugging state flow between providers and UI screens,
- maintaining a clean separation between UI, persistence, and sync logic,
- preventing brittle behavior when async operations complete after a screen has already changed state,
- keeping the app responsive even while offline or while waiting for network-backed sync.

Some exceptions and edge cases that can happen in this app include:

- a snackbar or UI update being attempted after a widget has already been disposed,
- sync failures caused by temporary connectivity loss or an unexpected API response,
- duplicate saves if a save action is triggered twice before the local state is refreshed,
- temporary stale counts in the user list if a refresh has not yet completed,
- unexpected background sync work after the device reconnects to the network.

For future maintenance, the safest approach is to keep provider logic simple, make database writes explicit, and validate offline and reload scenarios whenever new behavior is introduced.

## Notes

The project is currently structured as a mobile-first Flutter app with a strong focus on local persistence and responsive UI. It is well-suited for iterative feature expansion, especially around profile management, recommendations, and offline behavior.
